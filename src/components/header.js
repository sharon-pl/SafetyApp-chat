    ...
    render() {
      return(
        <View style={{...ifIphoneX({height: 80}, {height: 60}), width: responsiveWidth(100), backgroundColor: '#000', alignItems: 'center', flexDirection: 'row'}}>
            <TouchableHighlight onPress={()=>{this.props.prop.goBack()}} style={{width: responsiveWidth(10), height:40, ...ifIphoneX({hmargintop: 40}, {marginTop: 20}), marginLeft: 5}}>
                <Image source={Images.back} style={{width: 30, height: 30, marginTop: 5, tintColor: '#fff'}}></Image>
            </TouchableHighlight>
            <TouchableHighlight onPress={()=>{this.props.prop.navigate('HomeScreen')}} style={{width: responsiveWidth(80), height: 40, ...ifIphoneX({hmargintop: 40}, {marginTop: 20}), alignItems: 'center'}}>
